...
import { qcTestsApi } from '../services/api';
...
const generateQCCertificatePDF = async (testId: string): Promise<void> => {
  ...
  yPosition += 5;
  doc.text(`Phone: ${COMPANY_INFO.phone} | Email: ${COMPANY_INFO.email}`, margin, yPosition);
  yPosition += 10;
  ...
  // Table rows
  if (test.results) {
    // Extract specifications from test results (these come from Supabase)
    const meshSizeRequired = test.results.meshSize?.required || test.results.meshSize?.value || 'N/A';
    const purityMinRequired = test.results.purity?.minRequired || 95;
    const roundnessMinRequired = test.results.roundness?.minRequired || 0.8;
    const moistureMaxRequired = test.results.moisture?.maxRequired || 1.0;
    
    const results = [
      {
        parameter: 'Mesh Size',
        value: test.results.meshSize?.value || '',
        spec: meshSizeRequired,
        passed: test.results.meshSize?.passed || false,
      },
      {
        parameter: 'Purity (%)',
        value: (test.results.purity?.value || 0).toFixed(2) + '%',
        spec: `≥ ${purityMinRequired}%`,
        passed: test.results.purity?.passed || false,
      },
      {
        parameter: 'Roundness',
        value: (test.results.roundness?.value || 0).toFixed(2),
        spec: `≥ ${roundnessMinRequired.toFixed(2)}`,
        passed: test.results.roundness?.passed || false,
      },
      {
        parameter: 'Moisture (%)',
        value: (test.results.moisture?.value || 0).toFixed(2) + '%',
        spec: `≤ ${moistureMaxRequired}%`,
        passed: test.results.moisture?.passed || false,
      },
    ];
